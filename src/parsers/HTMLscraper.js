...
class HTMLScraper {
    ...
    getBookAbbreviations(setterFunction, finishedFunction, errorFunction) {
        ...
        this._request.onload = function () { //now 'this' refers to the XMLHttpRequest
            ...
            /* save the abbreviations to the HTMLScraper object so we can
                                                   retrieve them later
                                                */
            if (setterFunction) {
...
function getItemsBehindLink(htmlString) {
    ...
    var regex = new RegExp('<a href="([^"]+)">([^.<]+|\\d+[.]md)(?=</a>\\s*?</td>)', 'g');
    /* nasty regex that matches links only
    ...
    while (matches != null) {
        ...
        /* assigns the attribute tag with link;
